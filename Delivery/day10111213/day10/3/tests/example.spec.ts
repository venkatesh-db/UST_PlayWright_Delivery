...
test('Playwright Demo: Inputs & Text', async ({ page }) => {
  ...
  const smiles= await page.locator('#testing li').allInnerTexts();
  console.log('all inner happiens',smiles)

  const allInner = await page.locator('#testing li').allInnerTexts();
  const allText = await page.locator('#testing li').allTextContents();
